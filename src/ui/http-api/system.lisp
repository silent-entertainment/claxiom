...
;;;;;;;;;; System-level hooks
(define-json-handler (claxiom/system/kill-thread) ()
...
(define-json-handler (claxiom/system/formats) ()
  (hash :eval (front-end-eval-formats)
        :export (export-book-formats)))

(define-json-handler (claxiom/system/home-path) ()
  (namestring (user-homedir-pathname)))

(define-json-handler (claxiom/system/ls) ((dir :existing-directory))
  (let ((dirs nil)
        (files nil))
...
(define-handler (claxiom/source :close-socket? nil) ()
  (subscribe! :claxiom-updates sock))

(define-json-handler (claxiom/loaded-books) ()
  (mapcar
   (lambda (book)
...
(define-json-handler (claxiom/system/complete) ((partial :string) (package :package))
  (get-completions partial package))

(define-handler (claxiom/system/macroexpand-1 :content-type "plain/text") ((expression :string))
  (format nil "~s" (macroexpand-1 (read-from-string expression))))

(define-handler (claxiom/system/macroexpand :content-type "plain/text") ((expression :string))
  (format nil "~s" (macroexpand (read-from-string expression))))

(define-json-handler (claxiom/system/arg-hint) ((name :string) (package :package))
